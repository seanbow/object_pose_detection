...
public:
  ...
  Eigen::Matrix4d getPoseEstimate() { return pose_; }
...
private:
  ...
  void                 updateCovariance(std::vector<int32_t> &active);

  double *X,*Y,*Z;    // 3d points
  double *p_residual; // residuals (p_residual=p_observe-p_predict)

  // Eigen::MatrixXd dC_, C_; // estimate covariance
  Eigen::Matrix<double, 6, 6> dC_, C_;
