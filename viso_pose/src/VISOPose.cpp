...
VISOPose::VISOPose(double px_sigma)
    : px_sigma_(px_sigma), last_img0_seq_(-1), last_img1_seq_(-1),
      got_calibration_(false), pose_(Matrix::eye(4)),
      full_pose_(Matrix::eye(4)), nh_("viso_pose"),
      ...
      last_img_pose_(Eigen::Matrix4d::Identity()), initialized_(false),
...
void VISOPose::img0_callback(const sensor_msgs::ImageConstPtr &msg) {
...
void VISOPose::img1_callback(const sensor_msgs::ImageConstPtr &msg) {
...
void VISOPose::caminfo1_callback(const sensor_msgs::CameraInfo::ConstPtr &msg) {
  if (got_calibration_)
    return;

  viso_params_.calib.f = msg->P[0];
  viso_params_.calib.cu = msg->P[2];
  viso_params_.calib.cv = msg->P[6];
  viso_params_.base = std::fabs(msg->P[3] / msg->P[0]);
...
void VISOPose::setExtrinsics(const std::vector<double> &I_q_C,
...
void VISOPose::tryProcessNextImages() {
  ...
  // ROS_INFO_STREAM("Processing VISO images, t = " << msg0->header.stamp);
  ...
  // ROS_INFO_STREAM("Dims = [" << dims[0] << ", " << dims[1] << ", " << dims[2]
  // << "]");
  ...
  for (size_t i = 0; i < img0.rows; ++i) {
    memcpy(img0_data + i * img0.cols, img0.ptr(i), img0.cols * sizeof(uint8_t));
    memcpy(img1_data + i * img1.cols, img1.ptr(i), img1.cols * sizeof(uint8_t));
  }
  ...
  if (!initialized_) {
    ...
    // can't get a pose estimate or anything (obviously) so just give to viso
    // and set up times etc
  ...
  if (viso_->process(img0_data, img1_data, dims)) {
    ...
    // Current pose: " << std::endl; std::cout << viso_->getPoseEstimate() <<
    // std::endl << std::endl;
...
void VISOPose::publishTransform(const Eigen::Matrix4d &pose) {
